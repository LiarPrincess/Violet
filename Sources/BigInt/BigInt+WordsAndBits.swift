...
extension BigIntHeap {
  ...
  internal init(twoComplement: inout BigIntStorage) {
    ...
    let isPositive = hasMostSignificantBit0(value: last)
    if isPositive {
  ...
  internal func asTwoComplement() -> BigIntStorage {
    ...
    if self.isPositive {
      ...
      if let last = self.storage.last, hasMostSignificantBit1(value: last) {
    ...
    if let last = copy.last, hasMostSignificantBit0(value: last) {
