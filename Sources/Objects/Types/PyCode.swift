...
internal final class PyCode: PyObject,
  ...
  internal func hash() -> HashableResult {
    ...
    let name = hasher.hash(self.code.name)
    let qualifiedName = hasher.hash(self.code.qualifiedName)
  ...
  internal func repr() -> String {
    return "<code object \(self.code.name) at \(self.ptrString), " +
           "file '\(self.filename)', line \(self.code.firstLine)>"
  }
}
