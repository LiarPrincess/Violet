...
|setupLoop(loopEndLabel: LabelIndex)|Pushes a block for a loop onto the block stack.<br/>The block spans from the current instruction up until `loopEndLabel`.|
|forIter(ifEmptyLabel: LabelIndex)|`TOS` is an iterator. Call its `Next()` method.<br/>If this `yields` a new value, push it on the stack (leaving the iterator below it).<br/>If not then `TOS` is popped, and the byte code counter is incremented by delta.|
...
|continue(loopStartLabel: LabelIndex)|Continues a loop due to a continue statement.<br/>`loopStartLabel` is the address to jump to<br/>(which should be a `ForIter` instruction).|
...
|loadCellOrFree(cellOrFreeIndex: CellOrFreeIndex)|Loads the `cell` contained in slot `index` of the `cell` and `free` variable storage.<br/>Pushes a reference to the object the `cell` contains on the stack.|
|storeCellOrFree(cellOrFreeIndex: CellOrFreeIndex)|Stores `TOS` into the `cell` contained in slot `index` of the `cell` and `free` variable storage.|
|deleteCellOrFree(cellOrFreeIndex: CellOrFreeIndex)|Empties the `cell` contained in slot `index` of the `cell` and `free` variable storage.<br/>Used by the `del` statement.|
|loadClassCell(cellOrFreeIndex: CellOrFreeIndex)|Much like `LoadCellOrFree` but first checks the locals dictionary before consulting the `cell`.<br/>This is used for loading free variables in `class` bodies.|
...
|setupExcept(firstExceptLabel: LabelIndex)|Pushes a try block from a try-except clause onto the block stack.<br/>`firstExceptLabel` points to the first except block.|
|setupFinally(finallyStartLabel: LabelIndex)|Pushes a try block from a try-except clause onto the block stack.<br/>`finallyStartLabel` points to the finally block.|
