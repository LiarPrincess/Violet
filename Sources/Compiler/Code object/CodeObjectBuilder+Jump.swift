...
extension CodeObjectBuilder {
  ...
  public func emitJumpAbsolute(to label: Label, location: SourceLocation) throws {
    let index = try self.emitIndexWithExtendedArgIfNeeded(label, location: location)
  ...
  public func emitPopJumpIfTrue(to label: Label, location: SourceLocation) throws {
    let index = try self.emitIndexWithExtendedArgIfNeeded(label, location: location)
    try self.emit(.popJumpIfTrue(labelIndex: index), location: location)
  }

  /// Append a `popJumpIfFalse` instruction to code object.
  public func emitPopJumpIfFalse(to label: Label, location: SourceLocation) throws {
    let index = try self.emitIndexWithExtendedArgIfNeeded(label, location: location)
  ...
  public func emitJumpIfTrueOrPop(to label: Label, location: SourceLocation) throws {
    let index = try self.emitIndexWithExtendedArgIfNeeded(label, location: location)
    try self.emit(.jumpIfTrueOrPop(labelIndex: index), location: location)
  }

  /// Append a `jumpIfFalseOrPop` instruction to code object.
  public func emitJumpIfFalseOrPop(to label: Label, location: SourceLocation) throws {
    let index = try self.emitIndexWithExtendedArgIfNeeded(label, location: location)
