...
class BigIntHashTests: XCTestCase {
  ...
  private let smis = generateSmiValues(countButNotReally: 50)
  ...
  private var common: [BigInt] {
    ...
    for p in self.heaps {
      let heap = p.create()
      if let smi = heap.asSmiIfPossible(), smiSet.contains(smi.value) {
        result.append(self.create(smi.value))
      }
    }
  ...
  private func insert(_ set: inout Set<BigInt>, values: [Smi.Storage]) {
    for value in values {
      let int = self.create(value)
      set.insert(int)
    }
  ...
  private func insert(_ set: inout Set<BigInt>, values: [HeapPrototype]) {
    for value in values {
      let int = self.create(value)
      set.insert(int)
    }
  ...
  private func insert<S: Sequence>(
    ...
    values: S
  ) where S.Element == (Smi.Storage, UnicodeScalar) {
  ...
  private func insert<S: Sequence>(
    ...
    values: S,
    excluding: [BigInt]
  ) where S.Element == (HeapPrototype, UnicodeScalar) {
    for (value, char) in values {
      let int = self.create(value)
  ...
  private func create(_ value: Smi.Storage) -> BigInt {
    return BigInt(value)
  }
